...
const API = "/pajak/current-pajak/";

export async function getServerSideProps(context) {
  const { token } = await authPage(context);
  const { pajakId } = context.params;
  const pajakKategoriInfo = await fetcher(API + pajakId);
...
function UpdatePajak() {
  ...
  const onFinish = (values) => {
    axios
      .put("/pajak/update-pajak/" + data.pajakId, values)
    ...
    <div>
      <div className="mb-2">
        <Breadcrumb>
          ...
          <Breadcrumb.Item>Update Pajak</Breadcrumb.Item>
        </Breadcrumb>
      </div>
