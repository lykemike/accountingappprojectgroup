    ...
    <div>
      <div className="mb-2">
        <Breadcrumb>
          ...
          <Breadcrumb.Item>Update Pembelian</Breadcrumb.Item>
        </Breadcrumb>
      </div>
      ...
      <CardMax>
        <Formik
          initialValues={{
            ...
            email: data?.getHeaderPembelian?.email,
            alamat_perusahaan: data?.getHeaderPembelian?.alamat_perusahaan,
            tgl_transaksi: data?.getHeaderPembelian?.tgl_transaksi,
            akun_hutang_id: data?.getHeaderPembelian?.akun_hutang_supplier_id,
            tgl_jatuh_tempo: data?.getHeaderPembelian?.tgl_jatuh_tempo,
            syarat_pembayaran_id: data?.getHeaderPembelian?.syarat_pembayaran_id,
            no_ref_penagihan: data?.getHeaderPembelian?.no_ref_penagihan,
            detail: data?.getHeaderPembelian?.DetailPembelian,
            pesan: data?.getHeaderPembelian?.memo,
            file_attachment: [],
            akun_diskon_id: data?.getHeaderPembelian?.akun_diskon_pembelian_id,
            subtotal: data?.getHeaderPembelian?.subtotal,
            total_diskon: data?.getHeaderPembelian?.total_diskon,
            pajak_id: data?.getHeaderPembelian?.pajak_id,
            pajak_masukan_id: data?.getHeaderPembelian?.pajak?.pajak_masukan_id,
            pajak_persen: data?.getHeaderPembelian?.pajak_persen,
            total_pajak: data?.getHeaderPembelian?.total_pajak,
            sisa_tagihan: data?.getHeaderPembelian?.sisa_tagihan,
          }}
          onSubmit={async (values) => {
            ...
            axios
              .post("/pembelian/update-pembelian/" + data?.getHeaderPembelian?.id, formData)
                ...
                <Col md={{ span: 6 }}>
                  <Form layout="vertical">
                    <Form.Item label="Nomor Referensi">
                      <Input
                        value={values.no_ref_penagihan}
                        size="middle"
                        onChange={(e) => setFieldValue((values.no_ref_penagihan = e.target.value))}
                      />
                    </Form.Item>

                    <Form.Item label="No. Transaksi">
                      <Input value={`Purchase Invoice #${data?.getHeaderPembelian?.id}`} size="middle" disabled placeholder="Auto" />
                    </Form.Item>
                  </Form>
                </Col>
                ...
                <Col md={{ span: 6 }}>
                  <Form layout="vertical">
                    <Form.Item label="Tanggal Jatuh Tempo">
                      <DatePicker
                        value={moment(values.tgl_jatuh_tempo)}
                        style={{ width: "100%" }}
                        format={"YYYY/MM/DD"}
                        onChange={(e) => {
                            ...
                            values.detail.map((i, index) => (
                              <TableRow key={index}>
                                <TableCell style={{ width: 200 }}>
                                  <Select
                                    value={values.detail[index].akun_pembelian_id}
                                    showSearch
                                    optionFilterProp="children"
                                    placeholder="Select..."
                                    style={{ width: "100%" }}
                                ...
                                <TableCell style={{ width: 200 }}>
                                  <Input
                                    value={values.detail[index].deskripsi}
                                    style={{ width: "100%" }}
                                    size="middle"
                                    onChange={(e) => {
                                ...
                                >
                                  <InputNumber
                                    value={values.detail[index].kuantitas}
                                    min={0}
                                    style={{ width: "100%" }}
                                    size="middle"
                                    onChange={(e) => {
                                ...
                                >
                                  <InputNumber
                                    value={values.detail[index].harga_satuan}
                                ...
                                >
                                  <InputNumber
                                    value={values.detail[index].diskon}
                                    style={{ width: "100%" }}
                                    defaultValue={0}
                                    min={0}
                                    max={100}
