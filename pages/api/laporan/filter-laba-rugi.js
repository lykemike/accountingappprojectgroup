...
export default async (req, res) => {
  try {
    ...
    res.status(201).json({ message: "Filter laba rugi success", labaRugi: end_result, grand_total, bebanSelainBebanPajak: newArrayBeban });
  } catch (error) {
    res.status(400).json({ message: "Filter laba rugi failed", error });
  }
};
