...
export default async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: "Get buku besar success", bukuBesar: sortBy(end_result, "label"), debit: total_debit, kredit: total_kredit });
  } catch (error) {
    res.status(400).json({ message: "Get buku besar failed", error });
  }
};
