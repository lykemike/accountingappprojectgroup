...
export default async (req, res) => {
  ...
  try {
    ...
    const timeOnlys = moment().format(" HH:mm");
    const dateTime = moment(req.body.tgl_transaksi).format("YYYY/MM/DD") + timeOnlys;
    ...
    await prisma.jurnalPembelian.deleteMany({
      where: {
        header_pembelian_id: +pembelianId,
      },
    });
    ...
    await prisma.auditLog.create({
      data: {
        ...
        page: "/penjualan/update-pembelian",
        time: currentDatetime,
        action: "UPDATE",
        description: `${currentDatetime} | ${decodedAccessToken.first_name} has updated Purchase Invoice #${pembelianId}.`,
      },
    });

    res.status(201).json({ message: "Update pembelian success", pembelianId: pembelianId });
  } catch (error) {
    res.status(400).json({ message: "Update pembelian failed", error });
  }
};
