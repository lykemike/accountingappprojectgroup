...
const API = "/jurnal/get-akuns";

export async function getServerSideProps(context) {
  const { token } = await authPage(context);
  const akunsInfo = await fetcher(API);
...
function AddJurnal() {
  ...
  const [akunInfo, setAkunInfo] = useState(0);
  const data_akunkasbank = [];
  const onfind = (values) => {
    // console.log(values)C:\Users\asus\Documents\GitHub\rework-accounting\pages\api\jurnal\get_akunkasbank\[akunId].js
    axios
      .get("/jurnal/get_akunkasbank/" + values)
      .then(function (response) {
        message.success(response.data.message);
        // console.log(response.data)
        data_akunkasbank.push(response.data.getAkuns);
        // console.log(data_akunkasbank[0].sisa_saldo)
        setAkunInfo(data_akunkasbank[0].akun_id);
        console.log(akunInfo);
      })
    ...
    <div>
      <div className="mb-2">
        <Breadcrumb>
          ...
          <Breadcrumb.Item>Add Jurnal</Breadcrumb.Item>
        </Breadcrumb>
      </div>
      ...
      <CardMax>
        <Formik
          ...
          onSubmit={async (values) => {
            // console.log(values)
            axios
              .post("/jurnal/create-jurnal", values)
              .then(function (response) {
                message.success(response.data.message);
                Router.push(`/jurnal/view/` + response.data.latestHeaderJurnalId);
              })
              .catch(function (error) {
                message.error(error.response.data.message);
              });
